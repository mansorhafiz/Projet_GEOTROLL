...
void USBCheckBusStatus(void)
{
    ...
    if(usb_bus_sense == USB_BUS_ATTACHED)       // Is the USB cable attached and the
	{											// host actively driving Vbus high?
    ...
    else
    {
        if(UCONbits.USBEN == 1)                 // Is the module on?
            USBModuleDisable();                 // If it is on, disable it
												// so that the D+ or D- pull
												// up resistor no longer sources
												// current onto D+ or D-.
												// Note: You may get an "expression is always
												// true" warning in some configurations.
												// Ignore this warning, everything is ok.
